...
def main():
    ...
    if "embed3" in wanted:
        if feats is not None and y_true is not None:
            Z_gen = None
            # 1) direct per-fault file
            per_fault_path = os.path.join(rd, f"gen_f{args.fault}.npy")
            if _exists(per_fault_path):
                Z_gen = np.load(per_fault_path)
            else:
                # 2) combined selected faults file
                gen_selected_path = os.path.join(rd, "gen_selected.npy")
                if _exists(gen_selected_path):
                    try:
                        d = np.load(gen_selected_path, allow_pickle=True).item()
                        Z_gen = d.get(str(args.fault), None)
                    except Exception as e:
                        print(f"[WARN] embed3: could not read gen_selected.npy: {e}")
                # 3) legacy: full file
                if Z_gen is None:
                    gen_all_path = os.path.join(rd, "gen_all.npy")
